...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default='../results/DAVIS/', type=str, help='Path where to read dataset info')
    # parser.add_argument('--seg_path', default='D:/Download/DAVIS16/DAVIS16_flow_only', type=str, help='Path where to read flow-pred. masks')
    parser.add_argument('--seg_path', default='../results/checkpoint_2000_iou_[0.748]/', type=str,
                        help='Path where to read flow-pred. masks')
    parser.add_argument('--output_path', default='../results/output/', type=str, help='Path where to save segmentations')
    parser.add_argument('--finetune_dino', type=bool, default=True)
    parser.add_argment('--ftckpt_path', default=None, type=str, help='Path where to save finetuned checkpoints')
    ...
    for i, video_name in enumerate(video_list):
        ...
        if 'DAVIS' in dataset:
            video_dir = os.path.join(args.data_path, "JPEGImages/480p", video_name).replace("\\","/")
        else:
            video_dir = os.path.join(args.data_path, "JPEGImages", video_name).replace("\\","/")

        frame_list = read_frame_list(video_dir)

        middle_seg_path = os.path.join(args.seg_path, frame_list[-10].split('/')[-2],frame_list[-10].split('/')[-1].replace(".jpg", ".png")).replace("\\","/")
        middle_seg, seg_ori = read_seg(middle_seg_path, args.patch_size)
        
        print("Check frame temporal consistency by propagation")
        prop_start_frame, error_list = check_consistency(args, model, frame_list, video_dir, middle_seg, seg_ori, color_palette)
        ...
        first_seg_path_fwd = os.path.join(args.seg_path, frame_list_fwd[0].split('/')[-2],frame_list_fwd[0].split('/')[-1].replace("jpg", "png")).replace("\\","/")
