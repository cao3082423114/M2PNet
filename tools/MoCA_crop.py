...
category_dir = os.path.join(base_dir, "JPEGImages").replace("\\","/")
categories = sorted(os.listdir(category_dir))

#load annotations
anno = pd.read_csv(os.path.join(base_dir, "Annotations", "annotations.csv").replace("\\","/"))
...
for category in categories:
	...
	#load first image
	images = sorted(os.listdir(os.path.join(category_dir, category).replace("\\","/")))
	first_img_dir = os.path.join(category_dir, category, images[0]).replace("\\","/")
	...
	for i in range(min(num_images, 300)):
		#read image
		image_dir = os.path.join(category_dir, category, images[i]).replace("\\","/")
		...
		os.makedirs(os.path.join(base_dir, "Annotations_original", category).replace("\\","/"), exist_ok=True)
		save_dir = os.path.join(base_dir, "Annotations_original", category,'{:05d}.png'.format(i)).replace("\\","/")
		...
		#save cropped
		os.makedirs(os.path.join(base_dir, "Annotations_cropped", category).replace("\\","/"), exist_ok=True)
		os.makedirs(os.path.join(base_dir, "JPEGImages_cropped", category).replace("\\","/"), exist_ok=True)
		save_dir = os.path.join(base_dir, "Annotations_cropped", category,'{:05d}.png'.format(i)).replace("\\","/")
		cv2.imwrite(save_dir, anno_img)
		save_dir = os.path.join(base_dir, "JPEGImages_cropped", category,'{:05d}.jpg'.format(i)).replace("\\","/")
