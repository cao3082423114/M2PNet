...
save_dir = "../data/MoCA/MoCA_filtered"
...
for cat in filtered_cats:
	print(cat)
	images = sorted(os.listdir(os.path.join(category_dir, cat).replace("\\","/")))
	annos = sorted(os.listdir(os.path.join(annotation_dir, cat).replace("\\","/")))
	save1 = os.path.join(cat_save_dir, cat).replace("\\","/")
	save2 = os.path.join(anno_save_dir, cat).replace("\\","/")
	...
	for i in range(min(len(images),300)):
		...
		if i % 3 == 0:
			im = os.path.join(category_dir, cat, images[i]).replace("\\","/")
			anno = os.path.join(annotation_dir, cat, annos[i]).replace("\\","/")
			save_name1 = "{}.jpg".format(str(j).zfill(5))
			save_name2 = "{}.png".format(str(j).zfill(5))
			shutil.copy2(im, os.path.join(save1, save_name1).replace("\\","/"))
			shutil.copy2(anno, os.path.join(save2, save_name2).replace("\\","/"))
			j+=1
