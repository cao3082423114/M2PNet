...
def main(args):
    ...
    cout_csv = os.path.join(args.out_dir,'MoCA_results.csv').replace("\\","/")
    ...
    for video in video_names:
        if not os.path.exists(os.path.join(args.out_dir, video).replace("\\","/")):
            os.mkdir(os.path.join(args.out_dir, video)).replace("\\","/")
        res_path = os.path.join(args.masks_dir,video).replace("\\","/")
        ...
        success_s = [0, 0, 0, 0, 0]
        if args.resize:
            image = np.array(cv2.imread(os.path.join(args.MoCA_dir,'JPEGImages',video,'{:05d}.jpg'.format(0)).replace("\\","/")))
            H, W, _ = image.shape
