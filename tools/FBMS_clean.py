...
base_dir = '../data/FBMS/'
save_dir = '../data/FBMS_clean/'
os.makedirs(save_dir, exist_ok=True)
im_save_dir = '../data/FBMS_clean/JPEGImages'
os.makedirs(im_save_dir, exist_ok=True)
anno_save_dir = '../data/FBMS_clean/Annotations'
...
for vid in all_vids:
    if vid in train_vids:
        vid_dir = os.path.join(base_dir, "Trainingset", vid).replace("\\","/")
    elif vid in test_vids:
        vid_dir = os.path.join(base_dir, "Testset", vid).replace("\\","/")
    ...
    for i, im in enumerate(images):
    	img = os.path.join(vid_dir, im).replace("\\","/")
    	save_name = str(i+1).zfill(5) + ".jpg"
    	shutil.copy2(img, os.path.join(save1, save_name).replace("\\","/"))

    anno_dir = os.path.join(vid_dir, "GroundTruth").replace("\\","/")
    ...
    goal_annotation_fnames = [os.path.join(save2, str(n).zfill(5)).replace("\\","/") + '.png' for n in n_with_gt]
    annotation_fnames = [os.path.join(anno_dir, f).replace("\\","/") for f in annotation_fnames]
